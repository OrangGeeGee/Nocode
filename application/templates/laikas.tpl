...
<div class="is_time">
    <form action="?p=laikas&target=is&action=find_time" method="post">
        ...
        <div class="advanced-search"{if not $date_from and not $date_till} style="display:none;"{/if}>
             Laikotarpis Nuo:<p class="rounded main-input-block"><input {if isset($date_from)}value="{$date_from}" {/if}name="date_from" class="main-input datepicker is_from" type="text" id="from" size="10" data-help="Ieškomo laikotarpio pradžia"/></p>
             Iki: <p class="rounded main-input-block"><input {if isset($date_till)}value="{$date_till}" {/if}name="date_till" class="main-input datepicker is_till" type="text" id="until" size="10" data-help="Ieškomo laikotarpio pabaiga"/></p>
        </div>
    </form>
</div>
...
<div class="is_time">
    <form action="?p=laikas&target=repair&action=find_time" method="post">
        ...
        <div class="advanced-search"{if not $date_from and not $date_till} style="display:none;"{/if}>
             Laikotarpis Nuo:<p class="rounded main-input-block"><input {if isset($date_from)}value="{$date_from}" {/if}name="date_from" class="main-input datepicker repair_from" type="text" id="from" size="10" data-help="Ieškomo laikotarpio pradžia"/></p>
             Iki: <p class="rounded main-input-block"><input {if isset($date_till)}value="{$date_till}" {/if}name="date_till" class="main-input datepicker repair_till" type="text" id="until" size="10" data-help="Ieškomo laikotarpio pabaiga"/></p>
        </div>
    </form>
</div>       
{/if}
